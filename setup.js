...
const userConfig = {
  ...
  lossCircuitStreakCooldownHours: existingConfig.lossCircuitStreakCooldownHours ?? 0,
  lossCircuitRollingCooldownHours: existingConfig.lossCircuitRollingCooldownHours ?? 0,
  lossCircuitSingleCooldownHours: existingConfig.lossCircuitSingleCooldownHours ?? 0,
  lossCircuitRecoverySizePct: existingConfig.lossCircuitRecoverySizePct ?? 0.5,
  // Screening filters
  timeframe,
  category,
  minOrganic,
  ...
  maxTvl,
  minVolume,
  minVolumeActiveTvlRatio: existingConfig.minVolumeActiveTvlRatio ?? 0.02,
  minBinStep,
  maxBinStep,
  ...
  opportunityPollEnabled: existingConfig.opportunityPollEnabled ?? true,
  opportunityPollIntervalSec: existingConfig.opportunityPollIntervalSec ?? 45,
  opportunityDecisionMinIntervalSec: existingConfig.opportunityDecisionMinIntervalSec ?? 90,
  chartIndicators: {
    ...(existingConfig.chartIndicators || {}),
    enabled: existingConfig.chartIndicators?.enabled ?? true,
    entryPreset: existingConfig.chartIndicators?.entryPreset ?? "momentum_quality",
    exitPreset: existingConfig.chartIndicators?.exitPreset ?? "supertrend_break",
    rsiLength: existingConfig.chartIndicators?.rsiLength ?? 7,
    intervals: existingConfig.chartIndicators?.intervals ?? ["5_MINUTE", "15_MINUTE"],
    candles: existingConfig.chartIndicators?.candles ?? 298,
    rsiOversold: existingConfig.chartIndicators?.rsiOversold ?? 30,
    rsiOverbought: existingConfig.chartIndicators?.rsiOverbought ?? 80,
    entryRsiMin: existingConfig.chartIndicators?.entryRsiMin ?? 45,
    entryRsiMax: existingConfig.chartIndicators?.entryRsiMax ?? 72,
    requireAllIntervals: existingConfig.chartIndicators?.requireAllIntervals ?? true,
    entryFailClosed: existingConfig.chartIndicators?.entryFailClosed ?? true,
  },
  // LLM
