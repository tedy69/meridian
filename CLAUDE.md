...
- `spot-state.js` persists `venue`, `marketSource`, `priceSource`, and executable-quote outage timing. Non-DLMM positions use `getSpotExitQuote` with the tracked atomic amount and value expected/minimum output after Jupiter's estimated transaction costs. Never call `getActiveBin` for those positions. After 15 seconds without a quote, management must keep attempting an emergency close; maximum holding time also remains actionable while unpriceable. Old state defaults to the legacy active-bin path.
...
| `tools/chart-indicators.js` | 299 | `confirmIndicatorPreset({mint, side})`. Entry defaults to fail-closed `momentum_quality`: 5m+15m bullish Supertrend, rising candle, and balanced RSI. Also supports `supertrend_break`, `rsi_reversal`, `bollinger_reversion`, `rsi_plus_supertrend`, `supertrend_or_rsi`, `bb_plus_rsi`, `fibo_reclaim`, `fibo_reject`. Fetches from Agent Meridian `/chart-indicators/{mint}`. |
...
1. **Pre-checks**: `getMyPositions` + `getWalletBalances` in parallel. Skip if at `maxPositions` or fewer than `0.01` SOL remain after `gasReserve`; otherwise use the computed dynamic deploy amount. Each skip writes a `decision-log` entry.
2. **Top candidates**: `getTopCandidates({limit: 10})` — applies ALL hard filters (TVL, fee/TVL, volume/active-TVL, volatility, organic, holders, mcap, bin step, launchpad allow/block, token age, same-asset cooldowns, base mints already in use, dev blocklist), required fail-closed 5m+15m momentum confirmation, **and** PVP-rival detection (default: warn; `blockPvpSymbols: true` → hard filter).
...
|---|---|---|
| `risk` | `maxPositions`, `maxDeployAmount` | 1, 0.3 |
| `screening` | `excludeHighSupplyConcentration`, `minFeeActiveTvlRatio`, `minVolumeActiveTvlRatio`, `minTvl`, `maxTvl`, `minVolume`, `minOrganic`, `minQuoteOrganic`, `minHolders`, `minMcap`, `maxMcap`, `minBinStep`, `maxBinStep`, `maxVolatility`, `timeframe`, `category`, `minTokenFeesSol`, `requireTokenAudit`, `useDiscordSignals`, `discordSignalMode`, `avoidPvpSymbols`, `blockPvpSymbols`, `maxBotHoldersPct`, `maxTop10Pct`, `allowedLaunchpads`, `blockedLaunchpads`, `minTokenAgeHours`, `maxTokenAgeHours` | see `user-config.example.json` |
...
| `indicators` | `enabled`, `entryPreset`, `exitPreset`, `rsiLength`, `intervals`, `candles`, `rsiOversold`, `rsiOverbought`, `entryRsiMin`, `entryRsiMax`, `requireAllIntervals`, `entryFailClosed` | true, momentum_quality, supertrend_break, 7, ["5_MINUTE", "15_MINUTE"], 298, 30, 80, 45, 72, true, true |
