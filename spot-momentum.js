...
export function spotScreeningPolicy(spot = {}) {
  return {
    minLiquidityUsd: finite(spot.minLiquidityUsd) ?? 50_000,
    minVolume5mUsd: finite(spot.minVolume5mUsd) ?? 5_000,
    minVolumeLiquidityRatio: finite(spot.minVolumeLiquidityRatio) ?? 0.05,
    minOrganic: finite(spot.minOrganic) ?? 70,
    minHolders: finite(spot.minHolders) ?? 500,
    minMarketCapUsd: finite(spot.minMarketCapUsd) ?? 150_000,
    maxMarketCapUsd: finite(spot.maxMarketCapUsd) ?? 10_000_000,
    minTokenAgeMinutes: finite(spot.minTokenAgeMinutes) ?? 30,
    maxTokenAgeHours: finite(spot.maxTokenAgeHours) ?? 72,
    maxTop10Pct: finite(spot.maxTop10Pct) ?? 30,
    maxBotHoldersPct: finite(spot.maxBotHoldersPct) ?? 20,
    minPriceChange5mPct: finite(spot.minPriceChange5mPct) ?? 0.5,
    maxPriceChange5mPct: finite(spot.maxPriceChange5mPct) ?? 12,
    minVolumeChangePct: finite(spot.minVolumeChangePct) ?? 0,
    minBuySellVolumeRatio: finite(spot.minBuySellVolumeRatio) ?? 1.1,
